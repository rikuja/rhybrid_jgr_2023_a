...
#ifndef SEP_PARTICLE_PROPAGATOR_SHOCK_RK2_H
#define SEP_PARTICLE_PROPAGATOR_SHOCK_RK2_H
...
namespace sep {
   ...
   class ParticlePropagShockRK2: public ParticlePropagatorBase {
    public:
      ParticlePropagShockRK2();
    ...
    private:
      ...
      void calculateShockDriftAcceleration(pargrid::CellID blockLID,Real t_crossing,PARTICLE& particle,const Real d_shock);
      void binarySearchShockCrossing(PARTICLE& particle,Real* x,Real* y);
      void propagateParticle(PARTICLE& particle,Real* x,Real* y);
   };

   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   ParticlePropagatorBase* ParticlePropagShockMaker() {return new ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>();}
   
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::ParticlePropagShockRK2(): ParticlePropagatorBase() {
      initialized = false;
      splitAmount = 1;
   }

   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   bool ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::addConfigFileItems(ConfigReader& cr,const std::string& regionName) {
      return true;
   }

   /** Search approximate time of shock crossing. After this function returns, current 
    * simulation time for the particle, given by ParticlePropagShockRK2::t, is set to 
    * the approximate time of shock crossing, and ParticlePropagShockRK2::dt is set to 
    * sim->dt - dt, where dt is the timestep taken by this function.
    */
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   void ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::binarySearchShockCrossing(PARTICLE& initialState,Real* x,Real* y) {
	 ...
	 if (iters > 100) {
	    ...
	    std::cerr << "\t shock region is: " << simControl.shock->getShockRegion(t,initialState.state) << std::endl;
	    exit(1);
	 }
	 #endif
   ...
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   void ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::calculateShockDriftAcceleration(pargrid::CellID blockLID,Real t_crossing,
											      PARTICLE& particle,const Real d_shock) {
      #ifndef NDEBUG
      bool ok_prior = true;
      if (particle.state[particle::V_PAR] != particle.state[particle::V_PAR]) ok_prior = false;
      if (particle.state[particle::MU] != particle.state[particle::MU]) ok_prior = false;
      if (ok_prior == false) {
	std::stringstream ss;
	ss << "ERROR: NAN before calculateShockDriftAcceleration" << std::endl;
	ss << "POS:" << particle.state[0] << '\t' << particle.state[1] << '\t' << particle.state[2] << std::endl;
	ss << "V_PAR: " << particle.state[particle::V_PAR] << "\t MU:\t" << particle.state[particle::MU] << std::endl;
	std::cerr << ss.str();
	exit(1);
      }
      #endif
      ...
      simControl.shockAccelerator.solveShockEncounterAnalytic(plasma,p);
      ...
      #ifndef NDEBUG
      bool ok = true;
      if (particle.state[particle::V_PAR] != particle.state[particle::V_PAR]) ok = false;
      if (particle.state[particle::MU] != particle.state[particle::MU]) ok = false;
      if (ok == false) {
	std::stringstream ss;
        ss << "ERROR: NAN after solveShockEncounterAnalytic" << std::endl;
	ss << "POS:" << particle.state[0] << '\t' << particle.state[1] << '\t' << particle.state[2] << std::endl;
        ss << "V_PAR: " << particle.state[particle::V_PAR] << "\t MU:\t" << particle.state[particle::MU] << std::endl;
        ss << "V_trans_par: " << V_trans_par << std::endl;
	ss << "B: " << B[0] << '\t' << B[1] << '\t' << B[2] << std::endl;
	ss << "Shock n " << shockNormal[0] << '\t' << shockNormal[1] << '\t' << shockNormal[2] << std::endl;
	ss << "Shock V " << V_shock_SIM[0] << '\t' << V_shock_SIM[1] << '\t' << V_shock_SIM[1] << std::endl;
	ss << "Plasma V " << plasmaState.V_plasma_SIM[0] << '\t' << plasmaState.V_plasma_SIM[1] << '\t' << plasmaState.V_plasma_SIM[2] << std::endl;
	std::cerr << ss.str();
        exit(1);
      }
      #endif
   ...
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   bool ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::finalize() {
      bool success = true;
      initialized = false;
      return success;
   }
   ...
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   bool ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::initialize(Simulation& sim,SimulationClasses& simClasses,ConfigReader& cr,
									   const std::string& regionName,const ParticleListBase* plist) {
   ...
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   bool ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::propagateCell(pargrid::CellID blockLID,pargrid::DataID particleDataID,
									    const double* const coordinates,unsigned int N_particles) {
	...
	= particleInjectionBuffers.find(species.getName());
	 ...
	 if (trialRegion != initialRegion) {
	    ...
	    binarySearchShockCrossing(particleList[p],x,y);
	    ...
	    calculateShockDriftAcceleration(blockLID,t,particleList[p],d_shock2_initial);

            #ifndef NDEBUG
	    bool ok = true;
	    if (particleList[p].state[particle::V_PAR] != particleList[p].state[particle::V_PAR]) ok = false;
	    if (particleList[p].state[particle::MU] != particleList[p].state[particle::MU]) ok = false;
	    if (ok == false) {
	      std::stringstream ss;
	      ss << "ERROR: NAN after SDA" << std::endl;
	      ss << "POS:" << particleList[p].state[0] << '\t' << particleList[p].state[1] << '\t' << particleList[p].state[2] << std::endl;
	      ss << "V_PAR: " << particleList[p].state[particle::V_PAR] << "\t MU:\t" << particleList[p].state[particle::MU] << std::endl; 
	      std::cerr << ss.str();
	      exit(1);
	    }
	    #endif
	    ...
	    // Split particle that cross shock:
	    if (initialRegion == doubleshock::UPSTREAM) continue;
	    if (trialRegion == doubleshock::DOWNSTREAM) continue;
	    ...
	    // Insert splitted particles to injection buffer:
	    ...
	    if (newCell != pargrid::INVALID_CELLID) {
	       ...
	       for (uint32_t cntr=1; cntr<splitAmount; ++cntr) injBuffer->insert(newCell,particleList[p]);
	    }
   ...
   template<class SPECIES,class PARTICLE,class ACCELERATOR> inline
   void ParticlePropagShockRK2<SPECIES,PARTICLE,ACCELERATOR>::propagateParticle(PARTICLE& particle,Real* x,Real* y) {
